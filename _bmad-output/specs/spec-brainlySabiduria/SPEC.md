...
- **CAP-11 — Sembrado del Corpus** *(FR-23…FR-25)*
  - **intent:** El Corpus crece extrayendo candidatas desde las obras, no desde otros sitios de citas, de modo que la Procedencia venga con el texto en lugar de buscarse después. Lo ejecuta el editor o un agente.
  - **success:** La Fuente se recupera desde su URL y su metadato —obra, año, licencia— se deriva del documento recuperado, nunca de lo que teclee quien lanza la extracción; una Fuente cuya licencia no permita reutilización, o una URL fuera del conjunto admitido, no produce candidatas; el texto de cada candidata se localiza **literalmente** en el documento de su Fuente antes de publicarse, y si no se localiza permanece en revisión; publicar es siempre un acto explícito, nunca por acumulación de tiempo ni por ausencia de objeción; aprobar somete la candidata a las mismas reglas que cualquier alta de CAP-8; el lote es reanudable; la política de selección de objetivo es determinista y consultable, y el editor puede anularla con la anulación registrada.
...
- **La red vive en la cáscara exterior de `tools/`** (AD-22). `tools/lib/`, `src/lib/`, el esquema y las páginas son puros sobre datos ya recuperados, y **ningún paso del build descarga nada**: una construcción no puede pasar a depender de que un servidor ajeno esté vivo y diga hoy lo mismo que ayer.
- **El documento de la Fuente se versiona y el cotejo corre en el build** (AD-23). Vive en `corpus/fuentes/` —que no es una colección y sí lo lee el build—, cada Cita lo referencia, y una Cita cuyo texto no aparezca literalmente en él rompe la construcción. La puerta de admisión comprueba que la Procedencia *exista* (AD-1); esta comprueba que sea *cierta*, que es lo que a volumen separa un Corpus verificado de uno que solo lo afirma.
...
- **No inventamos texto y lo presentamos como real.** El sistema no crea ni parafrasea el texto de una Cita, ni compone prosa nueva sobre una persona real y la publica como si estuviera documentada. Seleccionar y transcribir literalmente de una edición identificable sí es admisible — es lo que hace CAP-11, lo ejecute el editor o un agente. La **semblanza de un Autor entra en la prohibición**: o procede de una fuente citable, o la escribe una persona que responde de ella.
- **No ponemos publicidad donde se lee, ni monetizamos antes de su umbral.** Ninguna unidad publicitaria aparece en la Página de Cita ni en la de Colección, y ningún Modelo se enciende por debajo de su Umbral de Activación. Un Modelo sin coste de superficie sí puede nacer de lo que la página ya publica —el enlace de afiliación sale de la Procedencia—, siempre que la atribución se lea igual encendido que apagado. Lo que se adelanta es el diseño, nunca el cobro.
...
- **El cotejo literal contra el documento de la Fuente basta para que el sembrado ejecutado por agentes no degrade SM-C1.** Es el supuesto que sostiene la apertura de CAP-11. Si resulta falso, la palanca es volver a poner una persona en la aprobación, nunca relajar el cotejo. Se falsa barato: auditar una muestra de lo sembrado por agente contra sus ediciones.
